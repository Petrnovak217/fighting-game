...
class Game {
  ...
  randomGame(){
    const p1= {
      strike: (p1, p2, attackDmg) => strike(p1, p2, attackDmg),
      heal: (p1) => Player.heal(p1),
    };

    const p2 = {
      strike: (p2, p1, attackDmg) =>strike(p2, p1, attackDmg),
      heal: (p2) => heal(p2),
    };
  ...
  play(p1, p2) {
    ...
    while (!this.isOver) {
      ...
      if (randomAction1 === 1) {
        this.randomGame().player1.strike(p1, p2,p1.attackDmg);
      } else {
        this.randomGame().player1.heal(p1);
      }

      if (randomAction2 === 1) {
        this.randomGame().player2.strike(p2, p1,p2.attackDmg);
      } else {
        this.randomGame().player2.heal(p2);
      }
...
let player1 = new Player("Andrej",100,15);
let player2 = new Player("Petr",100,15);
